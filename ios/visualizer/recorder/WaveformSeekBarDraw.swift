...
public class WaveformSeekBarDraw {
    public init() {}
    ...
    public func waveformDraw(fromAudioAt audioAssetURL: URL,
                             with configuration: WaveformSeekBarConfig.Configuration,
                             qos: DispatchQoS.QoSClass = .userInitiated,
                             completionHandler: @escaping (_ waveformDraw: UIImage?) -> ()) {
        guard let waveformAnalyzer = WaveformSeekBarAnalyzer(audioAssetURL: audioAssetURL) else {
    ...
    public func waveformDraw(from samples: [Float], with configuration: WaveformSeekBarConfig.Configuration) -> UIImage? {
...
extension WaveformSeekBarDraw {
    ...
    func draw(waveform samples: [Float], newSampleCount: Int, on context: CGContext, with configuration: WaveformSeekBarConfig.Configuration) {
...
// MARK: Draw generation
private extension WaveformSeekBarDraw {
    func render(from waveformAnalyzer: WaveformSeekBarAnalyzer,
                with configuration: WaveformSeekBarConfig.Configuration,
        ...
        waveformAnalyzer.samples(count: sampleCount, qos: qos) { samples in
            ...
            completionHandler(self.waveformDraw(from: dampenedSamples, with: configuration))
        }
    }
    
    private func draw(on context: CGContext, from samples: [Float], with configuration: WaveformSeekBarConfig.Configuration) {
    ...
    private func drawBackground(on context: CGContext, with configuration: WaveformSeekBarConfig.Configuration) {
...
// MARK: - Helpers
private extension WaveformSeekBarDraw {
    private func stripeCount(_ configuration: WaveformSeekBarConfig.Configuration) -> Int {
    ...
    private func stripeBucket(_ configuration: WaveformSeekBarConfig.Configuration) -> Int {
    ...
    private func dampen(_ samples: [Float], with configuration: WaveformSeekBarConfig.Configuration) -> [Float] {
    ...
    private func dampFactor(x: Float, count: Float, with dampening: WaveformSeekBarConfig.Dampening) -> Float {
